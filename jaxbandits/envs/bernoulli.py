...
from jaxbandits.envs.base import BanditEnv, BanditEnvStep, EnvState
...
class BernoulliBandits(BanditEnv):
    ...
    def create(cls, key: jax.random.KeyArray, arms: int):
        ...
        return cls(
            arms=arms,
            state=BernoulliBanditsState(
                arm_probs=arm_probs, optimal_action=jnp.argmax(arm_probs)
            ),
        )
    ...
    def regret(self, action: int) -> float:
        return (
            self.state.arm_probs[self.state.optimal_action]
            - self.state.arm_probs[action]
        )
